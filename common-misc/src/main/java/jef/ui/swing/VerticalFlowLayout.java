...
public class VerticalFlowLayout implements LayoutManager, java.io.Serializable {
	 ...
	 * <ul>
	 * <li><code>FlowLayout.LEFT</code> <li><code>FlowLayout.RIGHT</code> <li>
	 * <code>FlowLayout.CENTER</code> <li><code>FlowLayout.LEADING</code> <li>
	 * <code>FlowLayout.TRAILING</code>
	 * </ul>
	 * 
	 * @param align
			...
			for (int i = rowStart; i < rowEnd; i++) {
				...
				if (m.isVisible()) {
					if (ascent[i] >= 0) {
						...
						nonbaselineWidth = Math.max(m.getWidth(),
								nonbaselineWidth);
					}
				}
			}
			width = Math.max(maxAscent + maxDescent, nonbaselineWidth);
			baselineOffset = (width - maxAscent - maxDescent) / 2;
		}
		for (int i = rowStart; i < rowEnd; i++) {
			Component m = target.getComponent(i);
			if (m.isVisible()) {
				...
				if (useBaseline && ascent[i] >= 0) {
					cx = x + baselineOffset + maxAscent - ascent[i];
				} else {
					cx = x + (width - m.getWidth()) / 2;
				}
				if (ltr) {
					m.setLocation(cx, y);
				} else {
					m.setLocation(cx, target.getHeight() - y - m.getHeight());
				}
				y += m.getHeight() + vgap;
			}
		}
	...
	public void layoutContainer(Container target) {
		synchronized (target.getTreeLock()) {
			...
			for (int i = 0; i < nmembers; i++) {
				...
				if (m.isVisible()) {
					...
					if ((y == 0) || ((y + d.height) <= maxheight)) {
						...
						rowh = moveComponents(target, x, insets.top + vgap,
								rowh,maxheight - y,  start, i, ltr,
			...
			moveComponents(target,x, insets.top + vgap, rowh, maxheight - y, 
