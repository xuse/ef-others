...
public class RegexpPool {
     ...
     */
    public void add(String re, Object ret) throws JefException {
	add(re, ret, false);
    }
    ...
    */
    ...
    private void add(String re, Object ret, boolean replace) throws JefException {
	int len = re.length();
	RegexpNode p;
	if (re.charAt(0) == '*') {
	    p = suffixMachine;
	...
	if (p.result != null && !replace)
	    throw new JefException(re + " is a duplicate");
