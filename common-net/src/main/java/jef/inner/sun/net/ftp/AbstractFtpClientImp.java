...
import jef.inner.sun.misc.RegexpPool;
import jef.inner.sun.net.TelnetInputStream;
import jef.inner.sun.net.TelnetOutputStream;
import jef.inner.sun.net.TransferProtocolClient;
...
public abstract class AbstractFtpClientImp extends TransferProtocolClient {
	...
	/**
	 * @return if the networking layer should send ftp connections through a
	 *         proxy
	 */
...
//		System.out.println("reply code:"+lastReplyCode);
	 ...
	 * @exception FtpProtocolException
	 *                if an error occurs when issuing the PASV command to the
	 *                ftp server.
	 */
	protected Socket openPassiveDataConnection() throws IOException {
		 ...
		 * Here is the idea:
		 * 
		 * - First we want to try the new (and IPv6 compatible) EPSV command But
		 * since we want to be nice with NAT software, we'll issue the EPSV ALL
		 * cmd first. EPSV is documented in RFC2428 - If EPSV fails, then we
		 * fall back to the older, yet OK PASV command - If PASV fails as well,
		 * then we throw an exception and the calling method will have to try
		 * the EPRT or PORT command
		 */
		if (issueCommand("EPSV ALL") == FTP_SUCCESS) {
			...
			// The response string from a EPSV command will contain the port
			// number
			// the format will be :
	...
	/**
	 * Tries to open a Data Connection with the server. It will first try a
	 * passive mode connection, then, if it fails, a more traditional PORT
	 * command
	 * 
	 * @param cmd
	 *            the command to execute (RETR, STOR, etc...)
	 * @return the opened socket
	 * 
	 * @exception FtpProtocolException
	 *                if an error occurs when issuing the PORT command to the
	 *                ftp server.
	 */
	protected Socket openDataConnection(String cmd) throws IOException {
		...
		try {
			...
			if (issueCommand(portCmd) == FTP_ERROR || issueCommand(cmd) == FTP_ERROR) {
	 ...
	 *            the hostname of the ftp server
	 * @param port
	 *            the port to connect to (usually 21)
	 * 
	 * @exception FtpProtocolException
	 *                if connection fails
	 */
	 ...
	 * 
	 * @param user
	 *            Username to use at login
	 * @param password
	 *            Password to use at login or null of none is needed
	 * 
	...
	public TelnetInputStream get(String filename) throws IOException {
		...
		try {
			...
			/*
			 * Well, "/" might not be the file delimitor for this particular ftp
			 * server, so let's try a series of "cd" commands to get to the
			 * right place.
			 */
	 ...
	 * 
	 * @param path
	 *            pathname to the directory to list, null for current directory
	 * @return the <code>InputStream</code> to read the list from
	 ...
	 *            path of the directory to CD to
	 * 
	 * @exception <code>FtpProtocolException</code>
	 */
	...
	/**
	 * Send a No-operation command. It's usefull for testing the connection
	 * status
	 * 
	 * @exception FtpProtocolException
	 *                if the command fails
	 ...
	 *            Hostname of the FTP server
	 * @param port
	 *            port number to connect to (usually 21)
	 * 
	 * @exception FtpProtocolException
	 *                if the connection fails
	 */
	...
	protected void finalize() throws IOException {
		/**
		 * Do not call the "normal" closeServer() as we want finalization to be
